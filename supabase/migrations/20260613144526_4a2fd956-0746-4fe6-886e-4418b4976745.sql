...
-- Add owner-configurable minimum response time (anti credit farming "speed trap")
ALTER TABLE public.surveys
  ADD COLUMN IF NOT EXISTS min_response_seconds integer NOT NULL DEFAULT 15;

-- Sanity bounds enforced via trigger (CHECK is fine — immutable)
ALTER TABLE public.surveys
  DROP CONSTRAINT IF EXISTS surveys_min_response_seconds_bounds;
ALTER TABLE public.surveys
  ADD CONSTRAINT surveys_min_response_seconds_bounds
  CHECK (min_response_seconds BETWEEN 0 AND 600);

-- Protect from tampering after publish (keep field locked like other settings)
...
-- Use owner's speed-trap threshold; flag speed-runners for review
...
BEGIN
  ...
  IF _min_ms > 0 AND _server_duration_ms < _min_ms THEN
    INSERT INTO public.review_flags(user_id, type, details)
      VALUES (NEW.respondent_id, 'speed_trap',
              jsonb_build_object('survey_id', NEW.survey_id,
                                 'duration_ms', _server_duration_ms,
                                 'min_ms', _min_ms));
