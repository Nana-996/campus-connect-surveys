...
WITH expected AS (
  SELECT
    p.id,
    CASE WHEN p.user_type = 'student' THEN 10 ELSE 5 END AS required_bonus,
    COALESCE((
      SELECT SUM(cl.delta)::int
      FROM public.credit_ledger cl
      WHERE cl.user_id = p.id
        AND cl.wallet = 'earned'
        AND cl.reason IN ('signup_bonus', 'signup_bonus_repair')
    ), 0) AS existing_bonus
  FROM public.profiles p
), repair AS (
  SELECT id, required_bonus - existing_bonus AS delta
  FROM expected
  WHERE existing_bonus < required_bonus
), updated_profiles AS (
  UPDATE public.profiles p
  SET earned_credits = p.earned_credits + r.delta
  FROM repair r
  WHERE p.id = r.id
  RETURNING p.id, r.delta
)
INSERT INTO public.credit_ledger(user_id, wallet, delta, reason, expires_at)
SELECT id, 'earned', delta, 'signup_bonus_repair', now() + interval '30 days'
