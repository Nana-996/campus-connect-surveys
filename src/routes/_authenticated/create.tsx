  ...
  head: () => ({
    meta: [
      { title: "Create a survey — CampusVerify" },
      {
        name: "description",
        content:
          "Design a targeted student survey, set your tier, and publish to verified university respondents in minutes.",
      },
...
type Draft = {
  ...
  targetAge: string;
  targetInterests: InterestEntry[];
  responseGoal: string;
  expiresAt: string;
  allowGeneral: boolean;
  questions: Question[];
  respondentBonus: number;
...
function Create() {
  ...
  const [minResponseSeconds, setMinResponseSeconds] = useState<string>(
    d.minResponseSeconds ?? "15",
  );
  ...
  useEffect(() => {
    if (tier !== "pro" && respondentBonus !== 0) setRespondentBonus(0);
  }, [tier]);
  ...
  const goalNum = responseGoal
    ? Math.max(1, Math.min(tierMax, parseInt(responseGoal, 10) || tierMax))
    : tierMax;
      ...
      {lecturerId && (
        <div className="mt-4 rounded-2xl border-2 border-primary/40 bg-primary/5 p-4">
          ...
          <p className="mt-1 text-sm">
            This survey will be linked to <strong>{lecturerName ?? "the selected lecturer"}</strong>{" "}
            and tagged as an official evaluation. Standard credit cost still applies because you
            chose the custom builder.
          </p>
          <div className="mt-3 max-w-xs">
            <Label htmlFor="course-code" className="text-xs">
      ...
      <form onSubmit={submit} className="mt-8 space-y-6">
              ...
              return (
                ...
                >
                  ...
                  <p className="mt-3 text-xs font-bold">
                    {isGeneral ? T.cost * 2 : T.cost} credits
                  </p>
        ...
        <div className="rounded-3xl border border-foreground/15 bg-card p-6 space-y-4 shadow-paper">
          ...
          <div>
            ...
            <Input
              ...
              placeholder={
                isGeneral
                  ? "What people think about remote work"
                  : "Sleep habits among 2nd-year students"
              }
            />
          </div>
          <div>
          ...
          {tier === "basic" ? (
            <p className="text-xs text-muted-foreground italic">
              {isGeneral ? (
                <>
                  Basic surveys are open to the entire public — upgrade to{" "}
                  <button
                    type="button"
                    onClick={() => setTier("targeted")}
                    className="font-bold underline text-foreground"
                  >
                    Targeted
                  </button>{" "}
                  to filter by country, age & interests.
                </>
              ) : (
                <>
                  Basic surveys go out to your whole campus — upgrade to{" "}
                  <button
                    type="button"
                    onClick={() => setTier("targeted")}
                    className="font-bold underline text-foreground"
                  ...
                  to pick department, year & interests.
                </>
              )}
            </p>
          ) : (
            <div className="space-y-3">
              {isGeneral ? (
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Select
                      value={targetCountry}
                      onValueChange={(v) => setTargetCountry(v === "__any" ? "" : v)}
                    >
                      <SelectTrigger id="tc">
                  ...
                  <div>
                    ...
                    <Select
                      value={targetAge}
                      onValueChange={(v) => setTargetAge(v === "__any" ? "" : v)}
                    >
                      <SelectTrigger id="ta">
              ...
              <div>
                ...
                <p className="mt-1 text-[11px] text-muted-foreground">
                  Only respondents whose interests overlap will see this survey. Leave empty for no
                  filter.
                </p>
              </div>
            </div>
          ...
          <div className="border-t border-foreground/10 pt-4">
            ...
            <div className="mt-3 grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input
                  ...
                  max={new Date(Date.now() + 1000 * 60 * 60 * 24 * 30 * 6)
                    .toISOString()
                    .slice(0, 10)}
                  value={expiresAt}
            ...
            <div className="mt-4">
              <Label htmlFor="speed-trap" className="text-xs">
              ...
              />
              <p className="mt-1 text-[11px] text-muted-foreground">
                Anti-farming: responses submitted faster than this earn no credits and are silently
                flagged for review. Respondents don't see the threshold. Default 15s; set 0 to
                disable.
              </p>
            </div>
          </div>
            ...
            <label className="flex items-start gap-3 rounded-xl border border-foreground/15 bg-background/40 p-3 cursor-pointer">
              ...
              <div>
                ...
                <p className="text-[11px] text-muted-foreground">
                  On by default — students from other campuses and general users can find and answer
                  this survey. Uncheck to limit responses to your own campus only.
                </p>
              </div>
            </label>
          )}
        ...
        {tier === "pro" && (
          <div className="rounded-3xl border-2 border-primary/40 bg-primary/5 p-6 shadow-paper">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Pro perk — give each respondent extra credits on top of the standard +1 for completing
              your survey. Higher rewards attract more responses faster.
            </p>
            ...
            <div className="mt-3 flex items-center justify-between rounded-xl bg-background/60 px-3 py-2 text-xs">
              <span className="text-muted-foreground">
                ...
                <span className="font-bold text-foreground">
                  {1 + respondentBonus} credit{1 + respondentBonus === 1 ? "" : "s"}
                </span>{" "}
                per quality response
              </span>
            ...
            <p className="mt-2 text-[11px] text-muted-foreground">
              Max +3 bonus credits per response. The total reward pool ({respondentBonus} ×{" "}
              {goalNum} response goal = {bonusTotal} credits) is held from your balance at publish.
            </p>
          </div>
        )}
        ...
        {!canAffordTotal && (
          <p className="text-center text-xs font-medium text-destructive">
            ...
            {isGeneral ? (
              <a href="/buy-credits" className="underline">
                buy more credits
              </a>
            ) : (
        ...
        <Button
          type="submit"
          size="lg"
          disabled={submitting || !canAffordTotal}
